...
1. **脚本优先 (Script-First)** — Skills 封装思考，脚本封装执行。确定性操作用脚本，LLM 只做推理
...
| **本质** | 确定性执行 | LLM 推理引导 |
| **异常处理** | 可枚举（权限/超时/不存在 → 固定消息） | 需要情境判断（"这个报错我没见过"） |
| **准确度** | 100%（无歧义） | 取决于 model |
| **Token 消耗** | 0 | 每次调用消耗 |
| **示例** | `grep ERROR \| sort \| uniq -c \| sort -rn \| head` | "分析这些错误分布，判断根因" |
...
│   │  │  Discoverer  │   │  Evaluator (6关) │   │Bootstrapper││  │
│   │  └──────────────┘   └──────────────────┘   └────────────┘│  │
│   └───────────────────────────────────────────────────────────┘  │
│                                                                   │
│   ┌──────────────────────────────────────────────────────┐       │
...
通过: 是 → 🔧 生成脚本 + 薄 skill wrapper（不进入后续关卡）
...
| 关卡失败 | 替代路径 | 理由 |
|---------|---------|------|
| G0 (脚本优先) | 生成脚本 + 薄 wrapper | 确定性操作用脚本更快更准，不消耗 token |
